...
from app.services.crud import get_active_orders, clean_order_id
...
def status_node(state: dict) -> Dict[str, Any]:
    ...
    if not orders:
        ...
        send_text_message(state["phone_number"], t("STATUS_SINGLE_ORDER", lang, order_id=clean_order_id(order.order_id), status_name=status_str))
    else:
        msg = t("STATUS_MULTIPLE_ORDERS", lang)
        for o in orders:
            status_str = format_status_display(o.status)
            msg += f"• #{clean_order_id(o.order_id)}: {status_str}\n"
